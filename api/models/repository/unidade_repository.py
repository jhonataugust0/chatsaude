...
class UnidadeRepository:
  ...
  async def select_unity_from_id(self, unity_id) -> Dict[str, Unidade]:
    ...
    async with Connection() as connection:
      try:
        ...
        log = Logging(message)
        await log.warning('select_unity_from_id', None, error, 500, {'params': {'unity_id': unity_id}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=message
        )
  ...
  async def select_unity_from_district(self, district) -> Dict[str, Unidade]:
    ...
    async with Connection() as connection:
      try:
        ...
        log = Logging(message)
        await log.warning('select_unity_from_district', None, error, 500, {'params': {'unity_id': district}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=message
        )
  ...
  async def insert_new_unity(self, name: str) -> Dict[str, Unidade]:
    ...
    async with Connection() as connection:
      try:
        ...
        result_proxy = await connection.execute(select(Unidade).where(Unidade.id == query.id))
        ...
        log = Logging(message)
        await log.warning('insert_new_unity', None, error, 500, {'params': {'user_id': name}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=message
        )
  ...
  async def update_unity_from_id(self, unity_id: int, table: str, input_data: int) -> Dict[str, Unidade]:
    ...
    async with Connection() as connection:
      try:
        query = update(Unidade).where(Unidade.id == unity_id).values({ table: input_data}).returning(Unidade.id)
        result = await connection.execute(query)
        await connection.commit()
        id_unity = result.scalar_one()

        result_proxy = await connection.execute(select(Unidade).where(Unidade.id == id_unity))
        unity_dict = Unidade.as_dict(result_proxy.scalar_one())
        return unity_dict
        ...
        log = Logging(message)
        await log.warning('update_unity_from_id', None, error, 500, {'params': {'unity_id':unity_id, 'table': table, 'input_data': input_data}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=message
        )
  ...
  async def delete_unity_from_id(self, unity_id: int) -> bool:
    ...
    async with Connection() as connection:
      try:
        ...
        log = Logging(message)
        await log.warning('delete_schedule_from_user_id', None, error, 500, {'params': {'unity_id': unity_id,}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=message
        )
