...
from sqlalchemy import select, MetaData, update, delete, insert
...
class UserRepository:
  ...
  async def select_user_from_cellphone(self, cellphone) -> Dict[str, Usuario]:
    ...
    async with Connection() as connection:
      try:
        ...
        log = Logging(message)
        await log.warning('select_user_from_cellphone', None, error, 500, {'params': {'cellphone': cellphone}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=message
        )
  ...
  async def insert_new_user(self, telefone: int) -> Dict[str, Usuario]:
    ...
    async with Connection() as connection:
      try:
        ...
        result_proxy = await connection.execute(select(Usuario).where(Usuario.id == query.id))
        ...
        log = Logging(message)
        await log.warning('insert_new_user', None, error, 500, {'params': {'telefone': telefone}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=message
        )
  ...
  async def update_user_data(self, telefone, table, input_data) -> Dict[str, Usuario]:
    ...
    async with Connection() as connection:
      try:
        query = update(Usuario).where(Usuario.telefone == telefone).values({ table: input_data }).returning(Usuario.id)
        result = await connection.execute(query)
        await connection.commit()
        user_id = result.scalar_one()

        result_proxy = await connection.execute(select(Usuario).where(Usuario.id == user_id))
        user_dict = Usuario.as_dict(result_proxy.scalar_one())
        return user_dict
        ...
        log = Logging(message)
        await log.warning('update_user_data', None, error, 500, {'params': {'telefone':telefone, 'table': table, 'input_data': input_data}})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=message
        )
  ...
  async def delete(self, cellphone) -> bool:
    async with Connection() as connection:
        try:
          ...
          log = Logging(message)
          await log.warning('delete', None, error, 500, {'params': {'cellphone': cellphone}})
          raise HTTPException(
              status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
              detail=message
          )
