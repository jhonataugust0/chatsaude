...
async def validate_nascent_date(date: str):
  """
  ...
  try:
    ...
    await log.warning('validate_email', None, str(error), 500, {'params': {'date': date}})
    raise HTTPException(
...
async def validate_date_schedule(date_schedule):
  try:
    ...
    log = Logging(message_log)
    await log.warning('validate_email', None, str(error), 500, {'params': {'date_schedule': date_schedule}})
    raise HTTPException(
          status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
          detail=message_log
      )