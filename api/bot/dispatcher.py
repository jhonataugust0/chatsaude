...
from api.models.repository.especialidade_repository import EspecialidadeRepository
...
class BotDispatcher:
  ...
  async def message_processor(self, message: str, cellphone: int, user_dict: dict = None):
    """
  ...
  async def trigger_processing(self, bot_response: dict, number: int, user: dict = None):
    try:
  ...
  async def data_users_update_flow(self, user: dict, message: str):
    ...
    try:
      ...
      if int(user_flow['etapa_registro']) == 1:
        ...
        await user_entity.update_user_data(user['telefone'], 'bairro', str(message))
  ...
  async def data_schedule_consult_update_flow(self, user: dict, message: str):
    ...
    try:
      ...
      if int(user_flow['etapa_agendamento_consulta']) == 1:
        ...
        data_schedule = await consult_entity.select_data_schedule_from_user_cellphone(int(user['telefone']))
