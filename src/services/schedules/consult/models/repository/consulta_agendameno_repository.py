...
from sqlalchemy import (Date, MetaData, and_, delete, desc, func, select, text, tuple_,
                        update)
...
from ..entity.agendamentos_model import AgendamentoConsulta
...
class AgendamentoConsultaRepository:
    ...
    async def select_all(self) -> List[Dict[str, AgendamentoConsulta]]:
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = select(AgendamentoConsulta)
                result = await connection.execute(query)
                rows = result.fetchall()
                return [AgendamentoConsulta.as_dict(row) for row in rows]
    ...
    async def select_data_schedule_from_user_id(self, user_id: int) -> Dict[str, AgendamentoConsulta]:
        """
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = (
                    ...
                    .where(AgendamentoConsulta.id_usuario == user_id, AgendamentoConsulta.ativo == 1)
                    .order_by(desc(AgendamentoConsulta.id))
                    .limit(1)
                )
                result = await connection.execute(query)
                schedule_dict = AgendamentoConsulta.as_dict(result.scalar_one())
    ...
    async def select_all_data_from_schedule_with_id(self, id: int) -> Dict[str, Union[AgendamentoConsulta, Unidade, Especialidade]]:
        """
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = select(AgendamentoConsulta).where(AgendamentoConsulta.id == id)
                result = await connection.execute(query)
                schedule_data = AgendamentoConsulta.as_dict(result.scalar_one())
    ...
    async def select_data_schedule_from_user_cellphone(self, cellphone: int) -> Dict[str, AgendamentoConsulta]:
        """
            Busca no banco de dados a linha de dados do agendamento
            pertencente ao usuário informado
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = (
                    select(AgendamentoConsulta, Usuario)
                    .join(Usuario, AgendamentoConsulta.id_usuario == Usuario.id)
                    .where(Usuario.telefone == cellphone, AgendamentoConsulta.ativo == 1)
                    .order_by(AgendamentoConsulta.id.desc())
                )
                result = await connection.execute(query)
                row = result.fetchone()
                schedule = AgendamentoConsulta.as_dict(row.AgendamentoConsulta)
    ...
    async def insert_new_schedule_consult(self, user_id: int) -> Dict[str, AgendamentoConsulta]:
        """
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = AgendamentoConsulta(
                    id_usuario=user_id, ativo=1
                )
                connection.add(query)
                await connection.commit()
                result_proxy = await connection.execute(
                    select(AgendamentoConsulta).where(AgendamentoConsulta.id == query.id)
                )
                schedule_dict = AgendamentoConsulta.as_dict(result_proxy.scalar_one())
    ...
    async def insert_new_schedule_exam(self, user_id: int) -> Dict[str, AgendamentoConsulta]:
        """
    ...
    async def update_schedule_from_user_id(self, id_usuario: int, table: str, input_data: int) -> Dict[str, AgendamentoConsulta]:
        """
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = (
                    ...
                    .where(
                        AgendamentoConsulta.id_usuario == id_usuario, AgendamentoConsulta.ativo == 1
                    )
                    .values({table: input_data})
                    .returning(AgendamentoConsulta.id)
                )
                result = await connection.execute(query)
                await connection.commit()
                schedule_id = result.scalar_one()
                result_proxy = await connection.execute(
                    select(AgendamentoConsulta).where(AgendamentoConsulta.id == schedule_id)
                )
                schedule_dict = AgendamentoConsulta.as_dict(result_proxy.scalar_one())
                return schedule_dict
    ...
    async def update_schedule_from_id(self, id: int, table: str, input_data: int) -> Dict[str, AgendamentoConsulta]:
        """
            Atualiza uma coluna no banco de dados baseado no id do
            usuário informado
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                query = (
                    update(AgendamentoConsulta)
                    .where(AgendamentoConsulta.id == id, AgendamentoConsulta.ativo == 1)
                    .values({table: input_data})
                    .returning(AgendamentoConsulta.id)
                )
                result = await connection.execute(query)
    ...
    async def delete_schedule_from_user_id(self, user_id: int) -> bool:
        ...
        async with Connection(connection_url=self.connection_url) as connection:
            try:
                await connection.execute(AgendamentoConsulta.delete()).where(
                    AgendamentoConsulta.id == user_id
                )
                await connection.commit()
                return True
            except NoResultFound:
    ...
    async def get_last_time_scheduele_from_specialty_id(self, specialty_id: int, unity_id: int) -> Dict[str, AgendamentoConsulta]:
        """
